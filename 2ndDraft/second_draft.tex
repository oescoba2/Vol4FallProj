...
\usepackage{varwidth, caption} %enables \FloatBarrier, which prevents figures and tables from going below it.
...
This is why the Gompertz is a good modeling choice than other given models\ \cite{LairdGompertz}. 
...
The term $\frac{jD^2}{k + D^2}L$ is the number ofCD8$^+$ cells recruited to fight cancer, also related to  Michaelis-Menten kinetics.
...
To model the tumor, we first created a general function that would compute the initial conditions of the tumor.
Specifically, we called $t=0$ to be the time when a tumor should be of sufficient that a person, medical practice, or immune system should be able to identify the cancer.
While it is unknown why cancer can go undetected completely from the immune system, there are estimates for a tumor burden that should be recognizable by the immune system. 
...
Our modeling started about an order of magnitude less from the given estimate, that being a tumor burden of $T_o = 9^8$ (see Figure~\ref{fig:FullODE}).
...
It was part of our interest to see if our model would show a tumor growing without any intervention from the immune system.
This is in part why we picked our initial value of $T_o$.
As show in Figure~\ref{fig:FullODE}, our model does exhibit a growth, slow and leveling off to a $T_max$, prior to the beginning of a chemotherapy treatment plan. 
