...
%%BEGIN NEW ANALYSIS HERE

HERE HERE HERE HERE HERE HERE HERE HERE 
The system is not linear, so we cannot simply take the eigenvalues of the operator. We try to analyze the system by linearizing around the equilibrium solution. The equilibrium solution that makes the most sense is when there are no cancer cells or CD8+ cell, and a steady number of natural killer cells. This corresponds to T=0, L=0, N=sigma/N_d.  
Some nuance is necessary as there are zeros in the denominators at this point, but analysis shows that the limits properly exist. We take the Jacobian of the system of equations. 
...
Note that this cannot be evaluated at our equilibrium solution as log(Tmax/T) is not bounded as T->0. Even the linearization of the model fails to be stable, so it unfortunately cannot be trusted very much. 

%%END NEW ANALYSIS HERE
...
\begin{equation}
J = \begin{bmatrix}
\frac{\partial \dot{T}}{\partial T} & \frac{\partial \dot{T}}{\partial N} & \frac{\partial \dot{T}}{\partial L} \\
\frac{\partial \dot{N}}{\partial T} & \frac{\partial \dot{N}}{\partial N} & \frac{\partial \dot{N}}{\partial L} \\
\frac{\partial \dot{L}}{\partial T} & \frac{\partial \dot{L}}{\partial N} & \frac{\partial \dot{L}}{\partial L}
\end{bmatrix}
\end{equation}

Some specific partial derivatives, like:

\begin{equation}
\frac{\partial \dot{T}}{\partial T} = k_g \ln(\frac{T_{\max}}{T}) - k_g - n_{kr}N - \frac{\partial D}{\partial T}
\end{equation}

\begin{equation}
\frac{\partial D}{\partial T} = d\frac{(L/T)^\lambda}{s + (L/T)^\lambda} - d\lambda\frac{(L/T)^\lambda}{(s + (L/T)^\lambda)^2}\frac{L}{T^2}
\end{equation}
